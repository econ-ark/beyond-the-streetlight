...
cons_f = df['drconsum4'][153:]  

# Create DataFrames for each data source
date_df = pd.DataFrame({'DATE': date}).reset_index(drop=True)
unemp_df = pd.DataFrame({'SPF_UNEMPF4': unemp_f}).reset_index(drop=True)
headline_df = pd.DataFrame({'SPF_INFF4': headline_f}).reset_index(drop=True)
core_df = pd.DataFrame({'SPF_COREINFF4': core_f}).reset_index(drop=True)
cons_df = pd.DataFrame({'SPF_gRPCEF4': cons_f}).reset_index(drop=True)
